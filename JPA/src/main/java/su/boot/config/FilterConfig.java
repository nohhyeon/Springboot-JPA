...
public class FilterConfig {
	@Bean
	FilterRegistrationBean<CharacterEncodingFilter> characterEncodingFilter() { // 다이아몬드(< >) 연산자를 사용하면 제네릭 타입을 추론할 수
																				// 있다.
...
// 필터의 인코딩을 UTF-8로 설정한다.
		characterEncodingFilter.setEncoding("UTF-8"); // 인코딩 설정을 강제한다.
		characterEncodingFilter.setForceEncoding(true); // 등록 빈에 필터를 설정한다.
		filterRegistrationBean.setFilter(characterEncodingFilter); // 필터를 모든 URL 패턴에 대해 적용하도록 설정한다.
